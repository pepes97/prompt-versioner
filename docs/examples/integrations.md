...
jobs:
  test-prompts:
    ...
    - name: Install dependencies
      run: |
        pip install prompt-versioner pytest openai
...
"""Test prompt versions in CI/CD pipeline"""
...
def test_prompts():
    ...
    pv = PromptVersioner(project_name="ci_testing", enable_git=True)

    # Load prompts from files
    prompts_to_test = [
        {
            "name": "customer_service",
            "system_file": "prompts/customer_service_system.txt",
            "user_file": "prompts/customer_service_user.txt"
        }
    ]
    ...
    for prompt_config in prompts_to_test:
        ...
        # Read prompt files
        with open(prompt_config['system_file'], 'r') as f:
            system_prompt = f.read().strip()

        with open(prompt_config['user_file'], 'r') as f:
        ...
        # Run tests
        test_passed = run_prompt_tests(pv, prompt_config['name'], version_id)

        if not test_passed:
            all_tests_passed = False
            print(f"❌ Tests failed for {prompt_config['name']}")
        else:
            print(f"✅ Tests passed for {prompt_config['name']}")

    if not all_tests_passed:
        sys.exit(1)

def run_prompt_tests(pv, prompt_name, version_id):
    """Run specific tests for a prompt"""
    ...
    # Test cases
    test_cases = [
        {"input": "Simple test", "expected_min_quality": 0.7},
        {"input": "Complex test scenario", "expected_min_quality": 0.6}
    ]

    for test_case in test_cases:
        with pv.test_version(prompt_name, latest['version']) as test:
            # Simulate testing (replace with actual LLM calls)
            simulated_quality = 0.8  # Your actual testing logic here
            ...
            test.log(
                ...
                metadata={"test_case": test_case["input"]}
            )

            if simulated_quality < test_case["expected_min_quality"]:
                return False
...
- Log errors to both systems (Prompt Versioner + external)
